  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (key === 'orders_closed' || key === 'order_closed') {
        ...
        const parsed = Number(String(row[1] || '').trim())
        if (Number.isFinite(parsed) && parsed >= 0) settings.freeDeliveryFrom = parsed
      } else if (key === 'referral_percent_before_10') {
        const parsed = Number(String(row[1] || '').trim())
        if (Number.isFinite(parsed) && parsed >= 0) settings.referralPercentBefore10 = parsed
      } else if (key === 'referral_percent_after_10') {
        const parsed = Number(String(row[1] || '').trim())
        if (Number.isFinite(parsed) && parsed >= 0) settings.referralPercentAfter10 = parsed
      }
    }
...
export async function saveOrdersSettingsToSheet(sheetId: string, settings: OrdersSettings): Promise<void> {
  ...
  try {
    ...
    await updateOrAppend(deliveryFeeRowIndex, 'delivery_fee', String(settings.deliveryFee))
    await updateOrAppend(freeDeliveryFromRowIndex, 'free_delivery_from', String(settings.freeDeliveryFrom))
