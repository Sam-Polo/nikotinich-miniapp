    ...
    try {
      ...
      setFormData(prev => {
        ...
        return {
          ...
          image_keys: uploaded.key ? [...currentKeys, uploaded.key] : currentKeys
        }
      })
    } catch (err: any) {
  ...
  const handleDragEndForm = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      setFormData(prev => {
        ...
        if (oldIndex !== -1 && newIndex !== -1) {
          const keys = prev.image_keys || []
          return {
            ...prev,
            images: arrayMove(items, oldIndex, newIndex),
            image_keys: keys.length > 0 ? arrayMove(keys, oldIndex, newIndex) : keys
          }
        }
        return prev
      })
